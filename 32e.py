...
def exercise_32a():
    ...
    for k in range(1,2):
        ...
        for i in range(n+1):
            ...
            T=0.2+i*dn
            simulation = MDsimulator(T=1, nsteps=20000, dt=0.01, initial_spacing=size)
            simulation.simulate_animate()

            caplist[i]=np.average(simulation.P)
            tlist[i]=T

        stringer="Side lenght="+str(size)
        plt.plot(tlist, caplist, label=stringer)
