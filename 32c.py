...
class MDsimulator:
    ...
    def integrate_some_steps(self, framenr=None):
        ...
        if self.step >= self.startStepForAveraging and self.step % N_OUTPUT_HEAT_CAP == 0:
            self.heatcaplist.append(self.Cv)
...
def exercise_32a():
    for i in range(2):
        T=0.2+(i*0.8)
        nsteps = 20000
        simulation = MDsimulator(T=T, nsteps=20000, dt=0.01)
        simulation.simulate()
        stringer =str(T)
        #plt.plot(simulation.heatcaplist, label="Heat capacity for T=" + stringer)
        #plt.plot(simulation.epotList, label="potential energy for T=" + stringer)
        #plt.plot(simulation.etotList, label="total energy for T=" + stringer)

        if simulation.epotList[-1] > 100:
            break
